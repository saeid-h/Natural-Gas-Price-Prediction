...
%
%   eXogenicVariables - input time series.
%   HHSPm - feedback time series.

rng(142857,'twister')
load Variables.mat;

eXoVar = evalin('base', 'ExVar');
...
% Here all but 12 timesteps of the input series and target series are used
...
if false
    figure
    plot([10:228],cell2num(y))
    hold on
    plot(HHSPm)
    plot([229-12:228+12-12],cell2num(y2))
end
...
% nets = removedelay(adddelay(net,1));
...
if (true)
    ...
    % network performs.
    genFunction(net,'myNARXFunction');
...
%     [x,xi,ai,t] = preparets(net,X,{},T);
    ...
    for ii = 1:size(eXoVar_base,1)
        ...
        sgrowth = sfact * mean(eXoVar_base(ii,:));

        eXoVar(ii,:) = eXoVar_base(ii,:) * sfact;
        X = tonndata(eXoVar,true,false);
        [x,xi,ai,t] = preparets(net,X,{},T);
        y = myNARXFunction(x,xi,ai);
        ySA = cell2num(y);
        ...
        SADY(ii, 1) = nanmean((y_base(41:end) - ySA(41:end)) ./ ...
            ((sfact-1) .* eXoVar_base(ii,50:end))); 

        plot(Dates(10:end), y_base);
        datetick

        legend ([num2str(sfact) ' times of Base Case'], ...
        'Base Case', 'Location','northwest')
        xlabel('Date, year')
        ylabel ('Natural Gas Price, $/MMBtu')
        title (['Sensitivity Analysis for Variable No. ' num2str(ii)]);
    end
